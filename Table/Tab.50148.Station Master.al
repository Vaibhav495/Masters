...
table 50148 "Station Master"
{
    Caption = 'Station Master';
    DataCaptionFields = "No.", Name;
    DrillDownPageID = "Station Master List";
    LookupPageID = "Station Master List";
...
#pragma warning restore AS0086
        ...
        {
            CalcFormula = sum("Calendar Entry"."Capacity (Total)" where("Capacity Type" = const("Station Master"),
                                                                         "No." = field("No."),
        ...
        field(85; "Current Load in Qty"; Decimal)
        {
            FieldClass = FlowField;
            CalcFormula = sum(Artwork.Quantity where("Station Master" = field("No."), "In Machine" = filter(true)));
        }
        /*
    ...
    */
    ...
    begin
        SalesNRec.Get();
        if "No." = '' then begin
            SalesNRec.TestField("Station Master Nos.");
            Rec."No." := NoSeriesMgt.GetNextNo(SalesNRec."Station Master Nos.", Today, true); //Update 27.5
            //NoSeriesMgt.InitSeries(SalesNRec."Station Master Nos.", xRec."No. Series", 0D, "No.", "No. Series");
        end;
    end;
    ...
    procedure AssistEdit(OldSationMaster: Record "Station Master"): Boolean
    var
        IsHandled: Boolean;
    begin

        with StationMaster do begin
            StationMaster := Rec;
            SalesNRec.Get();
            SalesNRec.TestField("Station Master Nos.");
            //if NoSeriesMgt.SelectSeries(SalesNRec."Station Master Nos.", OldSationMaster."No. Series", "No. Series") then begin
